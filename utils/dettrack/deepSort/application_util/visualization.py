...
class Visualization(object):
    ...
    def __init__(self, seq_info, update_ms):
        ...
        self.image_shape = 1024, int(aspect_ratio * 1024)
        self.viewer = ImageViewer(update_ms, image_shape, "Figure %s" % seq_info["sequence_name"])
        self.viewer.thickness = 2
        self.frame_idx = seq_info["min_frame_idx"]
        self.last_idx = seq_info["max_frame_idx"]

    def run(self, frame_callback, video):
        self.viewer.run(lambda: self._update_fun(frame_callback), video)
